...
Google Search Console, Bing Webmaster Tools, 네이버 서치 어드바이저 등 검색 엔진 관리자 플랫폼에 도메인을 등록하였습니다.

이후 검색 엔진 봇은 JavaScript 실행 이전의 HTML을 기반으로 페이지를 분석할 수 있다는 점에 주목하여, Express 서버에서 요청 path에 맞는 title, description 등의 meta tag를 HTML 응답 단계에서 삽입하도록 구성하였습니다.

또한 Soft 404 문제를 방지하기 위해 페이지 상태에 맞는 HTTP 응답 코드를 반환하도록 구성하였으며, robots.txt와 sitemap.xml을 작성하여 검색 엔진 봇이 웹사이트 구조를 쉽게 파악할 수 있도록 개선하였습니다. 추가적으로 Lighthouse SEO 평가 항목을 분석하여 모바일 대응, canonical 설정 등 SEO 요소를 점검하였습니다.
...
브라우저별 UI 및 동작 차이를 최소화하여 일관된 사용자 경험을 제공하였으며, Safari 및 Android 네이버 앱 환경의 호환성 문제를 해결하고, 필요한 환경에서만 Polyfill을 로드하여 호환성과 초기 성능을 동시에 확보하였습니다.
...
Browser Use라는 오픈소스 도구를 통하여 Agentic AI를 활용한 브라우저 자동화를 구현하였습니다. 이때, 하나의 프롬프트에 많은 일을 시킨다면, AI가 업무를 잘 수행하지 못하는 이슈가 발생하였습니다. 이러한 이슈를 해결하기 위하여 업무의 단위를 잘게 나누어 프롬프트를 구성하였습니다. 더하여 프롬프트 엔지니어링을 통하여 완성도를 높혔습니다.
...
브라우저에서 발생하는 패킷을 캡쳐하고, 패킷에 문제가 없는지 혹은 표준에 부합하는지 검사하는 것이 목적입니다.

mitm 프록시를 통하여 브라우저의 패킷을 확인할 수 있습니다. 이러한 패킷에 취약한 점이 있는지, 혹은 표준에 부합하지 않는 부분은 없는지 검사합니다.
