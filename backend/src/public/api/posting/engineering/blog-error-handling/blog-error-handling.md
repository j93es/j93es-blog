...
안드로이드 네이버 앱의 경우 개인정보보호 브라우징과 웹사이트 공유하기 기능이 존재합니다. 그런데 2025년 2월 1일 기준, react-router-dom을 사용하며 path를 동적으로 생성하는 어플리케이션의 경우, 에러 핸들링을 할때에 `onerror` 이벤트 리스너 등록시, 공유하기 버튼을 누른다면 에러가 발생하여 정상적으로 어플리케이션이 동작하지 않았습니다.
