...
특히 애플 기기(Mac, iPhone 등)에서 긴 터치 제스처(트랙패드 뒤로가기 혹은 뒤로가기 제스처 등)에서 발생하는 화면 멈춤 현상을 브라우저 히스토리(popstate) 동작 분석을 기반으로 개선하였습니다. 더하여 안드로이드 네이버 앱 환경에서 `error` 이벤트 리스닝시에, 개인정보보호 브라우징 혹은 공유하기 기능에서 브라우저 콜백 에러로 인한 이슈를 해결하였습니다.
