...
build-frontend:
	sudo cp $(ENV_DIR)/frontend $(FRONTEND_DIR)/.env && cd $(FRONTEND_DIR) && sudo npm install && sudo npm run build && sudo mkdir $(BACKEND_DIR)/src/public/root/ && sudo cp -r $(FRONTEND_DIR)/build/* $(BACKEND_DIR)/src/public/root/
